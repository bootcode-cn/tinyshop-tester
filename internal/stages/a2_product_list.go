...
func productListTestCase() tester_definition.TestCase {
	return tester_definition.TestCase{
		...
		TestFunc: testProductList,
		CompileStep: autoCompileStep(
			"work/act2_stock_inventory/product_list.py",
			"tests/act2_stock_inventory/test_product_list.py",
		),
	}
}

func testProductList(harness *test_case_harness.TestCaseHarness) error {
	...
	tests := []tc{
		{"file_exists", "true", "work/act2_stock_inventory/product_list.py 文件存在"},
		{"exit_ok", "true", "脚本正常退出（exit code 0）"},
		{"output", "5|可乐|雪碧|红茶|咖啡|矿泉水", "输出 5 + 5 种饮料各一行"},
		{"uses_list_literal", "true", "用 list 字面量 products = [...] 定义"},
		{"uses_len", "true", "用 len(products) 算个数（不能硬编码 5）"},
	}
